...
inline void GenerateWavetables() {
    for (int f = 0; f < kWtFrames; ++f) {
        int harm = 1 << f;
        if (harm > 64) harm = 64;                            // brighter frames
        ...
        for (int i = 0; i < kWtLen; ++i) {
            ...
            {
                float s = 0.0f;
                for (int n = 1; n <= harm; ++n) s += sinf(n * ph) / n;
                g_wt[0][f][i] = s;
            }
            // 1: Square / pulse (odd harmonics, 1/n)
            {
                float s = 0.0f;
                for (int n = 1; n <= harm; n += 2) s += sinf(n * ph) / n;
                g_wt[1][f][i] = s;
            }
            // 2: Organ / additive (first f+1 harmonics at 1/sqrt(n))
            {
                float s = 0.0f;
                int H = f + 1;
                for (int n = 1; n <= H; ++n) s += sinf(n * ph) / sqrtf((float)n);
                g_wt[2][f][i] = s;
            }
...
class Voice {
  public:
    ...
    void SetWave(int wf) {
        for (int i = 0; i < kUni; ++i) osc_[i].SetWaveform(wf);
    }
    ...
    inline float Process() {
        ...
        if (engine < 0.5f) {  // ---- analog: super-saw unison + sub ----
            ...
            float pinc = f / sr_;
            wtPhase_ += pinc;
            wtPhase_ -= floorf(wtPhase_);
            float ph = wtPhase_;
            if (fmAmt > 0.001f) {  // FM through the sine frame (bank 0, frame 0)
                fmPhase_ += pinc * fmRatio;
                fmPhase_ -= floorf(fmPhase_);
