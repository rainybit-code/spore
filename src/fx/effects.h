...
static constexpr size_t kFxDelayMax = static_cast<size_t>(48000.0f * params::fx::kDelayBufSeconds);
...
class GlobalFx {
  public:
    ...
    void SetParams(float mix, float time, float fb, float tone, float decay, float damp) {
        ...
        if (sync_ > 0) {  // delay time locked to the clock at a musical division
            ...
            ds =
                daisysp::fmap(time, kDelayMinMs, kDelayMaxMs, daisysp::Mapping::EXP) * 0.001f * sr_;
        }
        if (ds < 1.0f) ds = 1.0f;
        if (ds > static_cast<float>(kFxDelayMax - 1)) ds = static_cast<float>(kFxDelayMax - 1);
        ...
        reverb_.SetLpFreq(daisysp::fmap(damp, kRevDampMinHz, kRevDampMaxHz, daisysp::Mapping::EXP));
    }
