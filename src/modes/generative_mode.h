...
class GenerativeMode : public IMode {
  public:
    ...
    void Control(Hothouse& hw, ModContext& ctx) override {
        ...
        cutoff_ =
            tone * (1.0f + (drift_ * 0.3f + wander * 0.6f) * ctx.mod.ChaosX());  // chaotic sway
        ...
        float scan = daisysp::fclamp(pScan_ + (drift_ * 0.15f + wander * 0.35f) * ctx.mod.ChaosY(),
                                     0.0f, 1.0f);
    ...
    void ProcessBlock(AudioHandle::InputBuffer /*in*/, AudioHandle::OutputBuffer out,
        ...
        for (size_t n = 0; n < size; ++n) {
            ...
            l *= 0.30f;
            r *= 0.30f;  // headroom for overlapping swells
            float wl, wr;
  ...
  private:
    ...
    void RollPatch() {
        ...
        // A random roll, WEIGHTED by the Brightness / Texture biases so re-seed keeps
        // surprising you within the character you've dialed in (GP_BRIGHT / GP_TEXTURE).
        ...
        pFmAmt_ =
            (rng_.Unipolar() < 0.20f + tx * 0.55f) ? rng_.Unipolar() * (0.12f + tx * 0.45f) : 0.0f;
        pFmRatio_ = kRatios[static_cast<int>(rng_.Unipolar() * 3.999f)];
        pFold_ =
            (rng_.Unipolar() < 0.12f + tx * 0.45f) ? rng_.Unipolar() * (0.20f + tx * 0.45f) : 0.0f;
        pDetune_ = 0.003f + rng_.Unipolar() * 0.012f;
        pRes_ = daisysp::fclamp(0.10f + rng_.Unipolar() * 0.30f + tx * 0.20f, 0.0f, 0.85f);
        pDrive_ =
            (rng_.Unipolar() < 0.30f + tx * 0.50f) ? rng_.Unipolar() * (0.30f + tx * 0.55f) : 0.0f;
        pFilter_ =
            (rng_.Unipolar() < 0.15f + tx * 0.25f) ? 1 : 0;  // mostly cheap Svf; Moog rare (CPU)
    ...
    void SeedEvent() {
        ...
        const float atk =
            (0.2f + swell * 3.5f) * (0.7f + 0.6f * rng_.Unipolar());  // stab .. long rise
        const float rel =
            (0.8f + swell * 10.0f) * (0.8f + 0.4f * rng_.Unipolar());  // short .. long fall
