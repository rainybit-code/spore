...
// =============================================================================
//  synth_mode.h  --  MODE 1: polyphonic MIDI synth.
// =============================================================================
...
class SynthMode : public IMode {
  public:
    ...
    void OnEnter() override {
        for (int i = 0; i < kVoices; ++i) v_[i].NoteOff();
        steal_ = 0;
    }
    ...
    void Control(Hothouse& hw, ModContext& ctx) override {
        ...
        static const int waves[4] = {
            daisysp::Oscillator::WAVE_SIN, daisysp::Oscillator::WAVE_POLYBLEP_TRI,
            daisysp::Oscillator::WAVE_POLYBLEP_SAW, daisysp::Oscillator::WAVE_POLYBLEP_SQUARE};
        int wf = waves[clampi(static_cast<int>(p.v[SP_WAVE] * 3.99f), 0, 3)];

        float cutoff = daisysp::fmap(ctx.knob[Hothouse::KNOB_1], kCutoffMinHz, kCutoffMaxHz,
                                     daisysp::Mapping::EXP);
        float res = ctx.knob[Hothouse::KNOB_2];  // 0..1; the Voice maps it per filter type
        float atk = daisysp::fmap(ctx.knob[Hothouse::KNOB_3], kAttackMinS, kAttackMaxS,
                                  daisysp::Mapping::EXP);
        float dec = daisysp::fmap(ctx.knob[Hothouse::KNOB_4], kDecayMinS, kDecayMaxS,
                                  daisysp::Mapping::EXP);
        ...
        float wtBank = (float)clampi(static_cast<int>(p.v[SP_WT_BANK] * (kWtBanks - 1) + 0.5f), 0,
                                     kWtBanks - 1);
        ...
        // ---- global LFO ----
        static const int lshapes[4] = {daisysp::Oscillator::WAVE_SIN, daisysp::Oscillator::WAVE_TRI,
                                       daisysp::Oscillator::WAVE_SAW,
                                       daisysp::Oscillator::WAVE_SQUARE};
        // LFO1 + LFO2 are matrix sources (route them via the patchbay); their rate
        // can itself be a destination (modulated by the previous block's mod).
        ...
        float depth1 =
            daisysp::fclamp(p.v[SP_LFO_DEPTH] + lfoKnob, 0.0f, 1.0f);  // LFO1 master depth
        ...
        // ---- mod matrix ----
        // sources: 1 LFO1 2 LFO2 3 Rnd 4 Sensor 7 Chaos 8 Steps (global) · 5 Velocity 6 Key
        // (per-voice) dests:   0 cutoff 1 pitch 2 scan 3 drive 4 sub 5 fm 6 amp 7 LFO1-rate 8
        // LFO2-rate
        float src[9] = {0.0f,
                        lfo * depth1,
                        lfo2 * p.v[SP_LFO2_DEPTH],
                        ctx.mod.Lfo1(),
                        ctx.sensors.Light() * 2.0f - 1.0f,
                        0.0f,
                        0.0f,
                        ctx.mod.ChaosX(),
                        ctx.mod.ChaosStep()};  // [5]/[6] per-voice (Vel/Key); [7] Chaos, [8] Steps
        ...
        int nPV = 0;
        int pvSrc[6], pvDst[6];
        float pvAmt[6];  // per-voice slots (Velocity / Key)
        for (int s = 0; s < 6; ++s) {
            ...
            if (si <= 4 || si >= 7) {
                mod[di] += src[si] * amt;
            }  // global source (incl. Chaos/Steps)
            else if (di <= 5) {
                pvSrc[nPV] = si;
                pvDst[nPV] = di;
                pvAmt[nPV] = amt;
        ...
        float scanMod = mod[2] * 0.5f, driveMod = mod[3] * 0.5f, subMod = mod[4] * 0.5f,
              fmMod = mod[5] * 0.5f;
        ...
        for (int i = 0; i < kVoices; ++i) {
            ...
            for (int k = 0; k < nPV; ++k) {
                float sv =
                    (pvSrc[k] == 5) ? v_[i].Vel() : (v_[i].Note() - 60.0f) / 24.0f;  // 5=vel, 6=key
                float a = sv * pvAmt[k];
                switch (pvDst[k]) {
                    case 0:
                        vc += a;
    ...
    void ProcessBlock(AudioHandle::InputBuffer /*in*/, AudioHandle::OutputBuffer out,
    ...
    void NoteOn(float note, float velocity) override {
        for (int i = 0; i < voices_; ++i)
            if (v_[i].Gate() && v_[i].Note() == note) {
                v_[i].NoteOn(note, velocity);
                return;
            }
        for (int i = 0; i < voices_; ++i)
            if (!v_[i].Active()) {
  ...
  private:
    ...
    static float LfoHz(float rateParam, float syncParam, float bpm) {
        ...
        static const float kCyclesPerBeat[5] = {0.25f, 0.5f, 1.0f, 2.0f,
                                                4.0f};  // 1/1 1/2 1/4 1/8 1/16
