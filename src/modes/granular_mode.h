...
static constexpr size_t kGranBufLen = static_cast<size_t>(48000.0f * params::granular::kBufSeconds);
...
class GranularMode : public IMode {
  public:
    ...
    void Control(Hothouse& hw, ModContext& ctx) override {
        ...
        grain_len_ =
            (kGrainSizeMinMs + k_size * (kGrainSizeMaxMs - kGrainSizeMinMs)) * 0.001f * sr_;
    ...
    void ProcessBlock(AudioHandle::InputBuffer in, AudioHandle::OutputBuffer out,
        ...
        for (size_t i = 0; i < size; ++i) {
            ...
            for (int g = 0; g < params::granular::kMaxGrains; ++g) {
                ...
                float w =
                    s_hann[static_cast<int>(gr.phase * kHannLen) & (kHannLen - 1)];  // Hann (LUT)
                w = fminf(1.0f,
                          w * shape_gain_);  // shape: scale + clip = soft Hann -> flat-top gate
            ...
            wetL *= 0.5f;
            wetR *= 0.5f;  // headroom for overlaps
  ...
  private:
    ...
    void SpawnGrain() {
        ...
        for (int g = 0; g < params::granular::kMaxGrains; ++g) {
            ...
            if (oct != 0)
                gr.inc = ldexpf(gr.inc, oct);  // exact x2^oct so octaves stay perfectly in tune
            if (rng_.Unipolar() < rev_)
                gr.inc = -gr.inc;  // reverse a fraction of grains (shimmer/texture)
    ...
    float ReadInterp(float pos) const {
        ...
        float frac = pos - i0;
        int i1 = i0 + 1;
        if (i1 >= static_cast<int>(kGranBufLen)) i1 = 0;  // branch wrap
